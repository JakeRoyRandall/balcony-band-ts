namespace BalconyBand {
  ...
  export function tapTempo(intervals: number[]): number | null { const valid = intervals.filter((interval) => Number.isFinite(interval) && interval >= 333 && interval <= 1000).slice(-5); if (valid.length === 0) return null; const sorted = [...valid].sort((a, b) => a - b); const middle = sorted.length % 2 ? sorted[Math.floor(sorted.length / 2)] : (sorted[sorted.length / 2 - 1] + sorted[sorted.length / 2]) / 2; return Math.max(60, Math.min(180, Math.round(60000 / middle))); }
  export function tapTempoState(timestamps: number[], now: number): { timestamps: number[]; bpm: number | null } { if (!Number.isFinite(now)) throw new RangeError('tap time must be finite'); let active = timestamps.length && now - timestamps[timestamps.length - 1] > 1500 ? [] : [...timestamps]; active.push(now); active = active.slice(-6); return { timestamps: active, bpm: tapTempo(active.slice(1).map((time, index) => time - active[index])) }; }
