...
const patternTools = document.createElement('div'); patternTools.className = 'pattern-tools'; patternTools.innerHTML = '<textarea id="pattern-json" aria-label="Pattern JSON" rows="3" placeholder="Pattern JSON lives here"></textarea><button id="export" class="transport">Export JSON</button><button id="import" class="transport">Import JSON</button><button id="download" class="transport">Download JSON</button><label class="file-upload" for="upload"><span>Upload JSON</span><input id="upload" type="file" accept="application/json,.json" aria-label="Upload pattern JSON"></label><button id="undo" class="transport" disabled>Undo</button><button id="redo" class="transport" disabled>Redo</button><div class="euclidean-tools"><label for="euclidean-voice">EUCLIDEAN</label><select id="euclidean-voice" aria-label="Euclidean voice"><option value="kick">Kick</option><option value="snare">Snare</option><option value="hat">Hat</option></select><label for="euclidean-pulses">PULSES</label><input id="euclidean-pulses" type="number" min="0" max="16" value="3" required aria-label="Euclidean pulses"><label for="euclidean-rotation">ROTATION</label><input id="euclidean-rotation" type="number" min="0" max="15" value="0" required aria-label="Euclidean rotation"><button id="euclidean-generate" class="transport">Generate</button></div><div class="save-tools"><label for="save-name">SAVE NAME</label><input id="save-name" maxlength="40" placeholder="e.g. Tuesday kitchen" aria-label="Save name"><button id="save" class="transport">Save</button><select id="save-slot" aria-label="Saved pattern"><option value="">Choose saved pattern…</option></select><button id="load" class="transport">Load</button><button id="delete-save" class="transport">Delete</button></div>'; feedback.parentElement!.insertBefore(patternTools, feedback);
const metronomeButton = document.createElement('button'); metronomeButton.id = 'metronome'; metronomeButton.className = 'transport'; metronomeButton.type = 'button'; metronomeButton.setAttribute('aria-pressed', 'false'); metronomeButton.textContent = 'Metronome off'; document.querySelector('.console-top')!.appendChild(metronomeButton);
const tapButton = document.createElement('button'); tapButton.id = 'tap-tempo'; tapButton.className = 'transport'; tapButton.type = 'button'; tapButton.textContent = 'Tap tempo'; tapButton.setAttribute('aria-label', 'Tap tempo twice or more to set BPM'); document.querySelector('.console-top')!.appendChild(tapButton);
const resetMixerButton = document.createElement('button'); resetMixerButton.id = 'reset-mixer'; resetMixerButton.className = 'transport'; resetMixerButton.type = 'button'; resetMixerButton.textContent = 'Reset mixer'; resetMixerButton.setAttribute('aria-label', 'Reset mixer settings without changing pattern'); document.querySelector('.console-top')!.appendChild(resetMixerButton);
...
function draw(): void {
  ...
  ($('tempo') as HTMLInputElement).value = String(tempo); ($('tempo') as HTMLInputElement).setAttribute('aria-valuenow', String(tempo));
  $('swing-value').textContent = `${swing}%`;
  const metronomeButton = $('metronome') as HTMLButtonElement; metronomeButton.textContent = metronomeEnabled ? 'Metronome on' : 'Metronome off'; metronomeButton.setAttribute('aria-pressed', String(metronomeEnabled));
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-solo]').forEach((button) => { const voice = button.dataset.solo as keyof BalconyBand.SoloState; const isSolo = solo[voice]; button.setAttribute('aria-pressed', String(isSolo)); button.setAttribute('aria-label', `${isSolo ? 'Unsolo' : 'Solo'} ${labels[voice].toLowerCase()}`); button.textContent = isSolo ? 'SOLO ON' : 'SOLO'; button.classList.toggle('active', isSolo); });
...
function metronomeClick(at: number, barStart: boolean): void { if (!audio) return; const oscillator = audio.createOscillator(); const gain = audio.createGain(); oscillator.frequency.value = barStart ? 1320 : 990; gain.gain.setValueAtTime(0.0001, at); gain.gain.exponentialRampToValueAtTime(barStart ? 0.045 : 0.028, at + 0.002); gain.gain.exponentialRampToValueAtTime(0.0001, at + 0.055); oscillator.connect(gain).connect(audio.destination); track(oscillator); oscillator.start(at); oscillator.stop(at + 0.065); }
function tick(): void {
  if (!running) return;
  if (!audio) return;
  ...
  const beat = BalconyBand.metronomeBeat(step); if (metronomeEnabled && beat.quarter) metronomeClick(nextAudioTime, beat.barStart);
  if (BalconyBand.voiceScheduled(muted, 'kick', velocities.kick, solo) && song.kick[step]) blip('kick', nextAudioTime); if (BalconyBand.voiceScheduled(muted, 'snare', velocities.snare, solo) && song.snare[step]) blip('snare', nextAudioTime); if (BalconyBand.voiceScheduled(muted, 'hat', velocities.hat, solo) && song.hat[step]) blip('hat', nextAudioTime);
...
$('download').addEventListener('click', () => { const blob = new Blob([BalconyBand.exportPattern(song, tempo)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'balcony-band-pattern.json'; link.click(); URL.revokeObjectURL(url); feedback.textContent = 'Pattern downloaded. The balcony has produced paperwork.'; });
 $('upload').addEventListener('change', async (event) => { const input = event.target as HTMLInputElement; const request = ++uploadRequest; const file = input.files?.[0]; if (!file) return; try { if (file.size > 65536) throw new RangeError('pattern file is limited to 64 KiB'); const raw = await file.text(); if (request !== uploadRequest) return; const loaded = BalconyBand.importPattern(raw); changePattern(loaded.song, loaded.tempo); feedback.textContent = 'Pattern uploaded. Press start when ready.'; } catch (error) { if (request === uploadRequest) feedback.textContent = `Upload failed: ${(error as Error).message}`; } finally { if (request === uploadRequest) input.value = ''; } });
...
$('reset-mixer').addEventListener('click', () => { if (running || starting) stop(); const defaults = BalconyBand.defaultMixerSettings(); Object.assign(muted, defaults.muted); Object.assign(solo, BalconyBand.emptySolos()); Object.assign(velocities, defaults.velocities); swing = defaults.swing; countInBars = defaults.countInBars; metronomeEnabled = defaults.metronome; ($('count-in') as HTMLSelectElement).value = String(countInBars); feedback.textContent = 'Mixer reset. Pattern, tempo, and saved arrangements are unchanged.'; draw(); });
$('tap-tempo').addEventListener('click', () => { const tapped = BalconyBand.tapTempoState(tapTimes, performance.now()); tapTimes = tapped.timestamps; if (tapped.bpm === null) { feedback.textContent = 'Tap tempo: tap once more to set the groove.'; return; } tempo = tapped.bpm; editHistoryState = { ...editHistoryState, present: { ...editHistoryState.present, tempo } }; feedback.textContent = `Tap tempo set to ${tempo} BPM.`; draw(); });
document.querySelectorAll<HTMLButtonElement>('[data-solo]').forEach((button) => button.addEventListener('click', () => { const voice = button.dataset.solo as keyof BalconyBand.SoloState; const next = BalconyBand.toggleSolo(solo, voice); Object.assign(solo, next); feedback.textContent = solo[voice] ? `${labels[voice]} soloed.` : `${labels[voice]} solo cleared.`; draw(); }));
